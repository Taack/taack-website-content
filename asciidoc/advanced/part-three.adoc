...
Let's start by implementing the `buildUsersTable` (you can use the author table as a base exemple to implement that.
...
Let's implement a `showUser` and `buildUserShow` methods in the controller and the UI service.
