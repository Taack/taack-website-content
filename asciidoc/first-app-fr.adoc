...
link:doc/plugin/taack-plugin-fr.adoc#_enregistrer_une_application[Enregistrer une application] pour en savoir plus sur la façon dont les plugins d'application sont implémentés sur le serveur.
...
. L'icône du bouton (doit être une valeur de l'enum xref:doc/Concepts/docref.adoc#_actionicon[ActionIcon]) ;
