= Collabora Editor
:doctype: book
:taack-category: 4|App
:toc:
:source-highlighter: rouge
:experimental: true

== Purpose

Extends Attachment Controller in Crew App.

Allow editing office document using https://www.collaboraonline.com/[Collabora Online]. All attachment become editable online.

== Roles

* `ROLE_TEMPLATE_ADMIN`: Allow users to create new template for to create new Attachment

== Install And Run Collabora CODE Docker Image

.Start Docker
[source,bash]
----
sudo systemctl restart docker
----

.Install Collabora CODE
[source,bash]
----
sudo docker pull collabora/code:latest
----

.Run Collabora CODE
[source,bash]
----
sudo docker run --privileged --network host -e "username=test" -e "password=test"  -e "extra_params=--o:ssl.enable=false" -p 9980:9980 collabora/code:latest   <1>
----

<1> For testing purpose only, we deactivate SSL and choose simple user / passwd. You do not need `--network host` if Collabora runs onto a different server.

.Test it is launched
[source,bash]
----
$ LANG=C wget http://localhost:9980/
--2025-02-18 22:35:56--  http://localhost:9980/
Resolving localhost (localhost)... ::1, 127.0.0.1
Connecting to localhost (localhost)|::1|:9980... connected.
HTTP request sent, awaiting response... 200 OK

    The file is already fully retrieved; nothing to do.
----

== Clone and link `editor-collabora` repo

[source,bash]
----
$ git clone https://github.com/Taack/collabora-editor.git
----

== Intranet Configuration

[source,yaml]
----
collabora.url: http://127.0.0.1:9980/     <1>
----

<1> Replace the IP address with the server dns name.

== Screenshot

.Editing an ODP
image::collabora-editor.webp[]

Some basic commands:

* Fullscreen => menu:Zoom[Fullscreen]
* Compact View =>  menu:Display[Compact View], or click on image:lc_sidebar.svg[width=25]
* Close Lateral Panel => menu:Display[Close Panel]
* Save file to create a new version on the Intranet (click on image:lc_save.svg[width=25] or press btn:[CTRL + S])


== Video

video::tB6mL07pzQQ[youtube,480]


